...
public class ClearConnectionsProcessor(
...
) : BaseUpdateProcessor(botProperties)
{
    public override Func<UpdateTypes, Update, SessionContext, bool> Filter => (type, update, context) =>
        ...
        (update.CallbackQuery?.Data?.StartsWith($"{CallbackQueriesConstants.ConnectionConfigurations};clear_connections;") ?? false) &&
    ...
    public override async Task Process(Update update, SessionContext sessionContext)
    {
        ...
        var deletedConnectionsCount = await connectionsRepository.DeleteByConfigurationIdAsync(configuration.Id);
        context.SelectedConfigurationId = configuration.Id;
        context.WaitingForSni = false;

        var text = $"Подключения для {configuration.ConnectionName} очищены.\nУдалено: {deletedConnectionsCount}";
