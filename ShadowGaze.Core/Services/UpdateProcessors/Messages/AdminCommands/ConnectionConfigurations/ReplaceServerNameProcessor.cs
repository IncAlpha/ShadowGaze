...
public class ReplaceServerNameProcessor(
...
) : BaseUpdateProcessor(botProperties)
{
    public override Func<UpdateTypes, Update, SessionContext, bool> Filter => (type, update, context) =>
        ...
        (update.CallbackQuery?.Data?.StartsWith($"{CallbackQueriesConstants.ConnectionConfigurations};replace_sni;") ?? false) &&
    ...
    public override async Task Process(Update update, SessionContext sessionContext)
    {
        ...
        context.WaitingForSni = true;

        var text = $"Текущий SNI: {configuration.ServerName}\nОтправь новый SNI для конфигурации {configuration.ConnectionName}";
